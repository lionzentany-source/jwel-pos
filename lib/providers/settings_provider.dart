...
final goldPriceProvider = Provider<double>((ref) {
  final materials = ref
      ...
      .maybeWhen(data: (list) => list, orElse: () => <app_mat.Material>[]);
...
final storeNameProvider = FutureProvider<String>((ref) async {
  ...
  return await repository.getStoreName() ??
      'My Store'; // Provide a default value
});
...
/// Whether POS should listen to USB HID/keyboard wedge scanners.
/// Defaults to true for desktop POS convenience.
final posKeyboardWedgeEnabledProvider = FutureProvider<bool>((ref) async {
  final repository = ref.read(settingsRepositoryProvider);
  return await repository.getBoolFlag(
    'pos_keyboard_wedge_enabled',
    defaultValue: true,
  );
});
...
class SettingsNotifier extends StateNotifier<AsyncValue<Map<String, dynamic>>> {
  ...
  Future<void> loadSettings() async {
    ...
    try {
      ...
      final silver = materials.firstWhere(
        (m) => m.nameAr == 'فضة',
        orElse: () =>
            app_mat.Material(nameAr: 'فضة', isVariable: true, pricePerGram: 5),
      );
  ...
  Future<void> updateGoldPrice(double price) async {
    // موقوف: الأسعار تُدار من خلال جدول المواد المتغيرة
  }
